...
public class DoctorPrescriptionController {
    ...
    public void updateStatistics() {
        try {
            ...
            rs = Database.query("SELECT COUNT(*) AS active FROM prescription p JOIN diagnosis dg ON p.DiagnosisID = dg.DiagnosisID JOIN appointment a ON dg.AppointmentID = a.AppointmentID WHERE a.Status = 'In-Progress'");
            if (rs != null && rs.next()) {
                InProgressAppointments.setText(String.valueOf(rs.getInt("active")));
            }

            // Patients
            rs = Database.query("SELECT COUNT(DISTINCT a.PatientID) AS patients FROM prescription p JOIN diagnosis dg ON p.DiagnosisID = dg.DiagnosisID JOIN appointment a ON dg.AppointmentID = a.AppointmentID");
            if (rs != null && rs.next()) {
                CanceledAppointments.setText(String.valueOf(rs.getInt("patients")));
            }

            // Prescribing Doctors
            rs = Database.query("SELECT COUNT(DISTINCT a.DoctorID) AS doctors FROM prescription p JOIN diagnosis dg ON p.DiagnosisID = dg.DiagnosisID JOIN appointment a ON dg.AppointmentID = a.AppointmentID");
            if (rs != null && rs.next()) {
    ...
    private void editPrescription(int prescriptionID) {
        try {
            ...
            String selectQuery = "SELECT dg.AppointmentID, dg.IllnessID, p.MedicineID, p.Dosage, a.Status, " +
