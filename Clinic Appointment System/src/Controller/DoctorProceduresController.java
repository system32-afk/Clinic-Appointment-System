...
public class DoctorProceduresController {
    ...
    private void updateStatistics() {
        try {
            ...
            // Scheduled (Pending from appointment)
            rs = Database.query("SELECT COUNT(*) AS scheduled FROM procedurerequest pr JOIN appointment a ON pr.AppointmentID = a.AppointmentID WHERE a.Status = 'Pending'");
            if (rs != null && rs.next()) {
                InProgressAppointments.setText(String.valueOf(rs.getInt("scheduled")));
            }

            // Completed
            rs = Database.query("SELECT COUNT(*) AS completed FROM procedurerequest pr JOIN appointment a ON pr.AppointmentID = a.AppointmentID WHERE a.Status = 'Completed'");
            if (rs != null && rs.next()) {
