...
public class EditProcedureController {
    ...
    private void loadDoctors() {
        ...
        String query = "SELECT DoctorID, CONCAT(FirstName, ' ', LastName) AS Name, SpecializationName FROM doctor d LEFT JOIN ref_specialization s ON d.SpecializationID = s.SpecializationID";
        try (ResultSet rs = Database.query(query)) {
            if (rs != null) {
                while (rs.next()) {
    ...
    private void loadExistingData() {
        String query = "SELECT pr.AppointmentID, pr.ServiceID, pr.ProcedureDate, pr.Status, " +
